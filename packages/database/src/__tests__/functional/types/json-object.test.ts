...
describe("Functional/Types/JsonObjectTest", () => {
  const functional = useFunctionalTestCase();

  beforeEach(async () => {
    await functional.dropAndCreateTable(
      Table.editor()
        .setUnquotedName("json_object_test_table")
        .setColumns(
          Column.editor().setUnquotedName("id").setTypeName(Types.INTEGER).create(),
          Column.editor().setUnquotedName("val").setTypeName(Types.JSON_OBJECT).create(),
        )
  ...
  it("insert and select", async () => {
    ...
    expect(await select(functional, 1)).toEqual(value1);
    expect(await select(functional, 2)).toEqual(value2);
  });
});
...
async function insert(
  ...
  const result = await functional.connection().insert(
    "json_object_test_table",
    {
      id,
      val: value,
    },
    [ParameterType.INTEGER, Type.getType(Types.JSON_OBJECT)],
  );
...
async function select(
  ...
  const value = await functional
    .connection()
    .fetchOne("SELECT val FROM json_object_test_table WHERE id = ?", [id], [ParameterType.INTEGER]);

  const decoded = functional.connection().convertToNodeValue(value, Types.JSON_OBJECT);
